...
def Wasserstein(box1, box2, xywh=True,eps = 1e-7):
    ...
    cx_L2Norm = torch.pow(((b1_x1 + b1_x2) / 2 - (b2_x1 + b2_x2) / 2), 2)
    cy_L2Norm = torch.pow(((b1_y1 + b1_y2) / 2 - (b2_y1 + b2_y2) / 2), 2)
    p1 = cx_L2Norm + cy_L2Norm + eps
    w_FroNorm = torch.pow((w1 - w2)/2, 2)
    h_FroNorm = torch.pow((h1 - h2)/2, 2)
    p2 = w_FroNorm + h_FroNorm
    return p1 + p2


def nwd_ratio(box1, box2, eps=1e-7):
    b2_x1, b2_y1, b2_x2, b2_y2 = box2.chunk(4, -1)
    w2, h2 = (b2_x2 - b2_x1).clamp(eps), (b2_y2 - b2_y1).clamp(eps)
    ciou = bbox_iou(box1, box2, xywh=False, CIoU=True)
    n_ratio = 0.22*(1-ciou*ciou)    #0.22*(1-ciou*ciou)                

    return n_ratio.clamp(eps)                    #torch.exp(-0.02*(w2+h2))*(1-ciou)                     #(0.1+0.9*pow((1-ciou),2))     #*(0.05+pow((1-ciou),2)) tmux2                   #*(0.05+pow((1-ciou),2))             
'''
def nwd_ratio(box1, box2, eps=1e-7):
    b2_x1, b2_y1, b2_x2, b2_y2 = box2.chunk(4, -1)
    w2, h2 = (b2_x2 - b2_x1).clamp(eps), (b2_y2 - b2_y1).clamp(eps)
    ...
    return torch.exp(-0.02*(w2+h2))*(pow((1-iou),2)*0.8+0.2) 
'''
...
def bbox_XIoU(box1, box2, xywh=True, GIoU=False, DIoU=False, XIoU=False, CIoU=False, NIoU=False, eps=1e-7):
    """
    ...
    if CIoU or DIoU or GIoU or XIoU:
        ...
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                return iou - (rho2 / c2 + v * alpha)  # CIoU    长宽比越大，中心距离权重越小，  iou越大则长宽比权重越大， 长宽比越大，长宽比权重越大
            return iou - rho2 / c2  # DIoU
        elif NIoU:
            c2 = cw ** 2 + ch ** 2 + eps  # convex diagonal squared
            rho2 = ((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4  # center dist ** 2
            ...
            v = (4 / math.pi ** 2) * (torch.atan(w2 / h2) - torch.atan(w1 / h1)).pow(2)
            with torch.no_grad():
                alpha = v / (v + torch.exp(-0.02*(w2+h2)) + eps)
